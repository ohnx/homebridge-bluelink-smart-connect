...
class BluelinkApiWrapper {
  ...
  constructor(log, username, password) {
    ...
    this.login((resp, err) => {if (!err) this.get_devices(null)});
  ...
  login(callback) {
    let login_details = {password: this.password, username: this.username};
    this.send_request('/api/v1/braeburn/rest-auth/login/', login_details, false, (response, err) => {
      ...
      this.cookie_jar = headers['set-cookie'].map(x => { x.split(';')[0] });
  ...
  get_devices(callback) {
    this.send_request('/api/v1/braeburn/devices/', null, true, (response, err) => {
      ...
      if (callback) callback(responseData[0], null);
    });
  }

  get_device_state(callback) {
    if (!this.device_id) return this.get_devices(callback);
    ...
    this.mutex
      ...
      if (rn - this.cache.device_state.time < 5000) {
        ...
        this.mutex.release();
        if (callback) callback(this.cache.device_state.data, null);
        return;
      }
